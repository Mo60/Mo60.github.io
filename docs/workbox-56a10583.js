define(['exports'], (t) => {
  'use strict'; try { self['workbox:core:7.0.0'] && _() }
  ...
  } const u = { googleAnalytics: 'googleAnalytics', precache: 'precache-v2', prefix: 'workbox', runtime: 'runtime', suffix: typeof registration != 'undefined' ? registration.scope : '' }; const l = t => [u.prefix, t, u.suffix].filter(t => t && t.length > 0).join('-'); const f = t => t || l(u.precache); const w = t => t || l(u.runtime); function d(t, e) { const s = e(); return t.waitUntil(s), s } try { self['workbox:precaching:7.0.0'] && _() }
  catch (t) {} function p(t) {
    if (!t)
      ...
      throw new s('add-to-cache-list-unexpected-type', { entry: t }); if (!e) { const t = new URL(n, location.href); return { cacheKey: t.href, url: t.href } } const i = new URL(n, location.href); const r = new URL(n, location.href); return i.searchParams.set('__WB_REVISION__', e), { cacheKey: i.href, url: r.href }
  ...
  } function v(t, e) { const s = new URL(t); for (const t of e)s.searchParams.delete(t); return s.href } class q {constructor() { this.promise = new Promise((t, e) => { this.resolve = t, this.reject = e }) }} const U = new Set(); try { self['workbox:strategies:7.0.0'] && _() }
  catch (t) {} function L(t) { return typeof t == 'string' ? new Request(t) : t } class b {
    constructor(t, e) { this.u = {}, Object.assign(this, e), this.event = e.event, this.l = t, this.p = new q(), this.g = [], this.R = [...t.plugins], this.m = new Map(); for (const t of this.R) this.m.set(t, {}); this.event.waitUntil(this.p.promise) } async fetch(t) {
      const { event: e } = this; let n = L(t); if (n.mode === 'navigate' && e instanceof FetchEvent && e.preloadResponse) {
    ...
    async fetchAndCachePut(t) { const e = await this.fetch(t); const s = e.clone(); return this.waitUntil(this.cachePut(t, s)), e } async cacheMatch(t) { const e = L(t); let s; const { cacheName: n, matchOptions: i } = this.l; const r = await this.getCacheKey(e, 'read'); const o = Object.assign(Object.assign({}, i), { cacheName: n }); s = await caches.match(r, o); for (const t of this.iterateCallbacks('cachedResponseWillBeUsed'))s = await t({ cacheName: n, matchOptions: i, cachedResponse: s, request: r, event: this.event }) || void 0; return s } async cachePut(t, e) {
      const n = L(t); let i; await (i = 0, new Promise(t => setTimeout(t, i))); const r = await this.getCacheKey(n, 'write'); if (!e)
    ...
    async getCacheKey(t, e) { const s = `${t.url} | ${e}`; if (!this.u[s]) { let n = t; for (const t of this.iterateCallbacks('cacheKeyWillBeUsed'))n = L(await t({ mode: e, request: n, event: this.event, params: this.params })); this.u[s] = n } return this.u[s] }hasCallback(t) {
    ...
    constructor(t = {}) { this.cacheName = w(t.cacheName), this.plugins = t.plugins || [], this.fetchOptions = t.fetchOptions, this.matchOptions = t.matchOptions }handle(t) { const [e] = this.handleAll(t); return e }handleAll(t) { t instanceof FetchEvent && (t = { event: t, request: t.request }); const e = t.event; const s = typeof t.request == 'string' ? new Request(t.request) : t.request; const n = 'params' in t ? t.params : void 0; const i = new b(this, { event: e, request: s, params: n }); const r = this.q(i, s, e); return [r, this.U(r, i, s, e)] } async q(t, e, n) {
    ...
    constructor(t = {}) { t.cacheName = f(t.cacheName), super(t), this._ = !1 !== t.fallbackToNetwork, this.plugins.push(E.copyRedirectedCacheableResponsesPlugin) } async L(t, e) { const s = await e.cacheMatch(t); return s || (e.event && e.event.type === 'install' ? await this.C(t, e) : await this.O(t, e)) } async O(t, e) {
      let n; const i = e.params || {}; if (!this._)
        throw new s('missing-precache-entry', { cacheName: this.cacheName, url: t.url }); { const s = i.integrity; const r = t.integrity; const o = !r || r === s; n = await e.fetch(new Request(t, { integrity: t.mode !== 'no-cors' ? r || s : void 0 })), s && o && t.mode !== 'no-cors' && (this.N(), await e.cachePut(t, n.clone())) } return n
    }

    async C(t, e) {
      this.N(); const n = await e.fetch(t); if (!await e.cachePut(t, n.clone()))
        throw new s('bad-precaching-response', { url: t.url, status: n.status }); return n
    }

    N() { let t = null; let e = 0; for (const [s, n] of this.plugins.entries())n !== E.copyRedirectedCacheableResponsesPlugin && (n === E.defaultPrecacheCacheabilityPlugin && (t = s), n.cacheWillUpdate && e++); e === 0 ? this.plugins.push(E.defaultPrecacheCacheabilityPlugin) : e > 1 && t !== null && this.plugins.splice(t, 1) }
  }E.defaultPrecacheCacheabilityPlugin = { cacheWillUpdate: async ({ response: t }) => !t || t.status >= 400 ? null : t }, E.copyRedirectedCacheableResponsesPlugin = { cacheWillUpdate: async ({ response: t }) => t.redirected ? await m(t) : t }; class O {
    constructor({ cacheName: t, plugins: e = [], fallbackToNetwork: s = !0 } = {}) { this.k = new Map(), this.K = new Map(), this.P = new Map(), this.l = new E({ cacheName: f(t), plugins: [...e, new g({ precacheController: this })], fallbackToNetwork: s }), this.install = this.install.bind(this), this.activate = this.activate.bind(this) } get strategy() { return this.l }precache(t) { this.addToCacheList(t), this.T || (self.addEventListener('install', this.install), self.addEventListener('activate', this.activate), this.T = !0) }addToCacheList(t) {
      const e = []; for (const n of t) {
        typeof n == 'string' ? e.push(n) : n && void 0 === n.revision && e.push(n.url); const { cacheKey: t, url: i } = p(n); const r = typeof n != 'string' && n.revision ? 'reload' : 'default'; if (this.k.has(i) && this.k.get(i) !== t)
          throw new s('add-to-cache-list-conflicting-entries', { firstEntry: this.k.get(i), secondEntry: t }); if (typeof n != 'string' && n.integrity) {
          if (this.P.has(t) && this.P.get(t) !== n.integrity)
            throw new s('add-to-cache-list-conflicting-integrities', { url: i }); this.P.set(t, n.integrity)
        } if (this.k.set(i, t), this.K.set(i, r), e.length > 0) { const t = `Workbox is precaching URLs without revision info: ${e.join(', ')}\nThis is generally NOT safe. Learn more at https://bit.ly/wb-precache`; console.warn(t) }
      }
    }

    install(t) { return d(t, async () => { const e = new y(); this.strategy.plugins.push(e); for (const [e, s] of this.k) { const n = this.P.get(s); const i = this.K.get(e); const r = new Request(e, { integrity: n, cache: i, credentials: 'same-origin' }); await Promise.all(this.strategy.handleAll({ params: { cacheKey: s }, request: r, event: t })) } const { updatedURLs: s, notUpdatedURLs: n } = e; return { updatedURLs: s, notUpdatedURLs: n } }) }activate(t) { return d(t, async () => { const t = await self.caches.open(this.strategy.cacheName); const e = await t.keys(); const s = new Set(this.k.values()); const n = []; for (const i of e)s.has(i.url) || (await t.delete(i), n.push(i.url)); return { deletedURLs: n } }) }getURLsToCacheKeys() { return this.k }getCachedURLs() { return [...this.k.keys()] }getCacheKeyForURL(t) { const e = new URL(t, location.href); return this.k.get(e.href) }getIntegrityForCacheKey(t) { return this.P.get(t) } async matchPrecache(t) { const e = t instanceof Request ? t.url : t; const s = this.getCacheKeyForURL(e); if (s) { return (await self.caches.open(this.strategy.cacheName)).match(s) } }createHandlerBoundToURL(t) {
  ...
  } let x; const N = () => (x || (x = new O()), x); class k extends i {constructor(t, e) { super(({ request: s }) => { const n = t.getURLsToCacheKeys(); for (const i of (function*(t, { ignoreURLParametersMatching: e = [/^utm_/, /^fbclid$/], directoryIndex: s = 'index.html', cleanURLs: n = !0, urlManipulation: i } = {}) { const r = new URL(t, location.href); r.hash = '', yield r.href; const o = (function (t, e = []) { for (const s of [...t.searchParams.keys()])e.some(t => t.test(s)) && t.searchParams.delete(s); return t }(r, e)); if (yield o.href, s && o.pathname.endsWith('/')) { const t = new URL(o.href); t.pathname += s, yield t.href } if (n) { const t = new URL(o.href); t.pathname += '.html', yield t.href } if (i) { const t = i({ url: r }); for (const e of t) yield e.href } }(s.url, e))) { const e = n.get(i); if (e) { return { cacheKey: e, integrity: t.getIntegrityForCacheKey(e) } } } }, t.strategy) }}t.NavigationRoute = class extends i {
    constructor(t, { allowlist: e = [/./], denylist: s = [] } = {}) { super(t => this.W(t), t), this.j = e, this.M = s }W({ url: t, request: e }) {
      if (e && e.mode !== 'navigate')
        return !1; const s = t.pathname + t.search; for (const t of this.M) {
        if (t.test(s))
          return !1
